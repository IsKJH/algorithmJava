...
// => 읽은 내용을 버퍼에 넣고 한 번에 처리하는 기능을 제공한다.
// => BufferedReader만 이용해서 파일에 쓰여진 데이터를 읽을 수 없으므로
...
//			br = new BufferedReader(fr); // FileReader를 이용해서 BufferedReader를 생성
			...
			while (true) {
				...
				if (line == null) { // read메소드는 ASCII를 반환하므로 읽을 내용이 없으면 -1을 반환하지만
					break; // readLine은 한 줄 텍스트를 읽어서 String으로 반환하므로 읽을 내용이 없으면 null을 리턴
				}
